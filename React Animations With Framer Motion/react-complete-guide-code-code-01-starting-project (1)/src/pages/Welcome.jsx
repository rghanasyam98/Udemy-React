...
export default function WelcomePage() {
  ...
  // to get how much we scrolled 
   const { scrollY }=useScroll();
  //  to change the opacity or image when scrolls
  const ycity=useTransform(scrollY,[0,200],[0,-150]);
  const opacityCity=useTransform(scrollY,[0,200,300,500],[1,0.5,0.5,0]);
  const yHero=useTransform(scrollY,[0,300],[0,-75]);
  const opacityHero=useTransform(scrollY,[0,300,400,500],[1,0.75,0.5,0]);
  const yText=useTransform(scrollY,[0,200,300,500],[0,50,150,300]);

  const textScale=useTransform(scrollY,[0,200,300,500],[1,1.1,1.2,1.3]);


  return (
    <>
      <header id="welcome-header">
        <motion.div id="welcome-header-content" style={{y:yText,scale:textScale}}>
